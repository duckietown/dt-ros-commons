...
class _DTROSDiagnosticsManager:
    ...
    def _compute_stats(self):
        ...
        # ---
        _conn_direction = lambda d: {"i": TopicDirection.INBOUND.value, "o": TopicDirection.OUTBOUND.value}[d]
        # connections stats
        ...
        for pub_stat in pub_stats:
            ...
            for conn in conn_stats:
                ...
                if _id not in connection_info:
                    continue
                link_info = {"bytes": _bytes, "messages": _num_messages, "dropped": 0, "_time": time.time()}
    ...
    def _publish_node_diagnostics(self):
        msg = DiagnosticsRosNode(**self._node_stats)
    ...
    def _publish_topics_diagnostics(self, *args, **kwargs):
        ...
        try:
            for topic, topic_stats in self._topics_stats.items():
                ...
                msg.topics.append(DiagnosticsRosTopic(node=rospy.get_name(), name=topic, **topic_stats))
        finally:
            self._topics_stats_lock.release()
    ...
    def _publish_parameters_diagnostics(self, force=False):
        ...
        for param, param_stats in self._params_stats.items():
            msg.params.append(NodeParameter(node=rospy.get_name(), name=param, **param_stats))
        self._params_diagnostics_pub.publish(msg)

    def _publish_links_diagnostics(self, *args, **kwargs):
        if (
            not self._links_diagnostics_pub.anybody_listening()
            and not self._topics_diagnostics_pub.anybody_listening()
        ):
            return
        self._compute_stats()
