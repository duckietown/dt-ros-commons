...
def _list():
    try:
        return response_ok({"nodes": sorted(KnowledgeBase.get("/node/list", []))})
        ...
        return response_ok(
            {
                "node": "/" + node,
                "topics": {
                    t_name: {"direction": t_info["direction"]}
...
def _params(node):
    try:
        return response_ok(
            {"node": "/" + node, "parameters": KnowledgeBase.get("/node/params/%s" % node, {})}
        )
    except Exception as e:
        return response_error(str(e))
...
def _services(node):
    try:
        return response_ok(
            {"node": "/" + node, "services": KnowledgeBase.get("/node/services/%s" % node, {})}
        )
    except Exception as e:
        return response_error(str(e))
