...
def _list():
    try:
        return response_ok({"topics": sorted(KnowledgeBase.get("/topic/list", []))})
...
def _find(msg_type):
    try:
        return response_ok({"message_type": msg_type, "topics": rt.find_by_type(msg_type)})
    except Exception as e:
        return response_error(str(e))
...
def _bw(topic):
    ...
    try:
        ...
        # return
        return response_ok({"topic": "/" + topic, "bandwidth": bw, "secs_since_update": bw_time})
    except Exception as e:
        return response_error(str(e))
...
def _publishers(topic):
    ...
    try:
        return response_ok(
            {"topic": topic, "publishers": KnowledgeBase.get("/topic/publishers%s" % topic, [])}
        )
    except Exception as e:
        return response_error(str(e))
...
def _subscribers(topic):
    ...
    try:
        return response_ok(
            {"topic": topic, "subscribers": KnowledgeBase.get("/topic/subscribers%s" % topic, [])}
        )
    except Exception as e:
        return response_error(str(e))
...
def _dttype(topic):
    ...
    try:
        return response_ok(
            {"topic": topic, "type": KnowledgeBase.get("/topic/type%s" % topic, default_topic_type(topic))}
        )
    except Exception as e:
        return response_error(str(e))
