...
def _list():
    try:
        return response_ok({"services": sorted(sorted(KnowledgeBase.get("/service/list", [])))})
...
def _type(service):
    ...
    try:
        ...
        if _service_type:
            return response_ok({"service": service, "message_type": _service_type})
        else:
...
def _find(msg_type):
    try:
        return response_ok({"message_type": msg_type, "services": rs.rosservice_find(msg_type)})
    except Exception as e:
        return response_error(str(e))
...
def _providers(service):
    ...
    try:
        return response_ok(
            {"service": service, "providers": KnowledgeBase.get("/service/providers%s" % service, [])}
        )
    except Exception as e:
        return response_error(str(e))
