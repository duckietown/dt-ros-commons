...
def _graph():
    ...
    try:
        ...
        graph = {
            "nodes": nodes.keys(),
            "edges": {"node_to_topic": [], "topic_to_node": [], "node_to_node": [], "topic_to_topic": []},
        }
        # collect edges (node -> topic)
        graph["edges"]["node_to_topic"].extend(
            [{"from": n, "to": t} for t in topics for n in KnowledgeBase.get(topic_key("publishers", t), [])]
        )
        # collect edges (topic -> node)
        graph["edges"]["topic_to_node"].extend(
            [{"from": t, "to": n} for t in topics for n in KnowledgeBase.get(topic_key("subscribers", t), [])]
        )
            ...
            for n in KnowledgeBase.get(topic_key("publishers", t0), []):
                for t1, t1_info in KnowledgeBase.get(node_key("topics", n), {}).items():
                    if t1_info["direction"] == TopicDirection.INBOUND.name:
                        edges.add((t1, n, t0))
        graph["edges"]["topic_to_topic"] = [{"from": t0, "middle": n, "to": t1} for t0, n, t1 in edges]
            ...
            for t, t_info in KnowledgeBase.get(node_key("topics", n0), {}).items():
                ...
                if t_info["direction"] == TopicDirection.INBOUND.name:
                    for n1 in KnowledgeBase.get(topic_key("publishers", t), []):
                        edges.add((n1, t, n0))
        graph["edges"]["node_to_node"] = [{"from": n0, "middle": t, "to": n1} for n0, t, n1 in edges]
        # ---
        return response_ok({"graph": graph, "nodes": nodes, "topics": topics})
